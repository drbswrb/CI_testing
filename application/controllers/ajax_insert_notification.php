...
class Ajax_insert_notification extends CI_Controller {
	...
	public function index()
	{
		...
		$query = mysql_query("insert into group_notification (group_id,notification,posted_by_username,posted_date) values($group_id,'{$notification_content}','{$posted_by}','{$date}')");
		$query2 = $this->db->query("select * from group_notification where group_id=$group_id order by posted_date desc");
		$row1=$query2->result();
		$i=0;
